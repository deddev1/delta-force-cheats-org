...
for (const width of outputWidths) {
	...
	await writeFile(path.join(imagesDir, `delta-force-cheats-hero-${width}w.webp`), webp);
	console.log(`✓ delta-force-cheats-hero-${width}w.webp (${width}x${height}, ${Math.round(webp.length / 1024)}KB)`);
}
...
for (const name of ['delta-force-cheats-hero.webp', 'delta-force-hero-banner.webp', 'hero-banner.webp']) {
	await writeFile(path.join(imagesDir, name), canonical);
}

const png = await resizeHero(lcpWidth).png({ compressionLevel: 6 }).toBuffer();
await writeFile(path.join(imagesDir, 'delta-force-cheats-hero.png'), png);
